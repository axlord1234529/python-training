...
from bokeh.palettes import Spectral11,Spectral4
from bokeh.models.sources import AjaxDataSource


def get_pandas_edge_list() -> pd.DataFrame:
    csv_file_path = 'display/data/D13_graph4.csv'
...
def data_endpoint(request):
    ...
    data = {
        ...
        'fill_color' : [Spectral11[random_number] for i in range(18)]
    }
...
def bokeh_handler(doc: Document) -> None:
    ...
    ajax_source = AjaxDataSource(data_url='http://127.0.0.1:8000/display/data', polling_interval=5000)
    ...
    # )
    graph.node_renderer.data_source = ajax_source
    ...
    def p_call(event):
        graph.node_renderer.data_source.data['fill_color'] = [Spectral4[3] for i in range(len(G))]
        print('I ran')

    # graph.on_event('tap', p_call)
    ...
    # p.on_event('tap',p_call)

    def my_callback(attr, old_selected, new_selected):
        print("Selected indices changed from", old_selected, "to", new_selected)

    # graph.edge_renderer.data_source.selected.on_change('indices', my_callback)
